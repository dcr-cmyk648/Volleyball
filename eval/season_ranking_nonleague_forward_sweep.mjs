// Forward-only sweep for Stats > Season > Season Ranking non-league knobs.
//
// League handling is intentionally fixed at the current app Season Ranking
// settings. This evaluates only non-league model/scoring choices against
// future non-league game outcomes.
...
function replayFor({ ratingOptions, volleyballOptions, updateContextMode }, priorGames = games) {
...
function buildScoringContext({ prior, priorGames, game, ratingOptions, volleyballOptions, scoringContextMode }) {
...
function computeForwardQuality({ ratingOptions, volleyballOptions, updateContextMode, scoringContextMode }) {
  ...
  sortedGames.forEach(game => {
    if (isQualityGame(game)) {
      const prior = replayFor({ ratingOptions, volleyballOptions, updateContextMode }, priorGames);
...
function addVolleyballOneAtATime(group, name, values) {
  values.forEach(value => addCandidate(`${name}=${String(value)}`, group, {
    volleyballOptions: { [name]: value },
  }));
}

addCandidate('current season ranking', 'baseline');

parseListEnv('SWEEP_SEASONAL_TAPER_DAYS', [90, 120, 150, defaultSeasonalTaperDays, 240, 365, 548, 730])
  .forEach(seasonalTaperDays => addCandidate(`age-out ${seasonalTaperDays}d`, 'age-out', {
    ratingOptions: { seasonalTaperDays },
  }));
...
function printRows(title, sortedRows, baseline, limit = printLimit) {
  ...
  console.log('-'.repeat(labelWidth + 112));
...
printRows('Best by forward Brier', [...rows].sort(compareBrier), baseline);
